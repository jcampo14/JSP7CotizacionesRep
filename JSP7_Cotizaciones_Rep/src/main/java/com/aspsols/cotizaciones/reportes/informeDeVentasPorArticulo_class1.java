...
public class informeDeVentasPorArticulo_class1 extends JReportRuntime {
	...
	protected Query queryQUERY0 = new Query("QUERY0","MASTER","WHERE FE.FAC = FD.FAC AND FE.C_EMP = FD.C_EMP AND FE.C_AGR = FD.C_AGR AND FE.N_IDE = FD.N_IDE AND FE.EST =  'A'  AND AR.C_EMP = FE.C_EMP AND FD.COD = AR.COD AND SUBSTR(FE.PER_CT, 1 , 6 ) BETWEEN SUBSTR(#periodo_ini, 1 , 6 ) AND SUBSTR(#periodo_fin, 1 , 6 )","GROUP BY AR.NOM,FD.COD,FE.PER_CT,FE.MON","ORDER BY FE.PER_CT"," SELECT  AR.NOM   ,FD.COD   ,FE.PER_CT   ,FE.MON   ,SUM( ROUND (FD.CAN * FD.VEN, 2 ))  VENTAS   FROM FAC_DET FD, FAC_ENC FE, ARTICULO AR  WHERE FE.FAC = FD.FAC AND FE.C_EMP = FD.C_EMP AND FE.C_AGR = FD.C_AGR AND FE.N_IDE = FD.N_IDE AND FE.EST =  'A'  AND AR.C_EMP = FE.C_EMP AND FD.COD = AR.COD AND SUBSTR(FE.PER_CT, 1 , 6 ) BETWEEN SUBSTR(#periodo_ini, 1 , 6 ) AND SUBSTR(#periodo_fin, 1 , 6 )  GROUP BY AR.NOM,FD.COD,FE.PER_CT,FE.MON",null);
